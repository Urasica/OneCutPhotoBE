...
public class CrowdBasedPhotoController {
    ...
    @Operation(
            summary = "홈화면 사진용 관광지 목록 조회(집중률 낮은 순)",
    ...
    @Operation(
            summary = "홈화면 사진용 관광지 목록 조회(집중률 높은 순)",
            description = "요청 시마다 랜덤하게 섞인 관광지 목록을 반환합니다."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "정상 응답"),
    ...
    @GetMapping("/recommend-high")
    public ResponseEntity<List<CrowdBasedPhotoHigh>> getPhotosHigh() {

        List<CrowdBasedPhotoHigh> photos = photoService.getCrowdBasedPhotosHigh();
        return ResponseEntity.ok(photos);
    }
}
